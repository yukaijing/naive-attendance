...
class Miner {
  ...
  mine(rewardAddress, feeAddress) {
    let baseBlock = Miner.generateNextBlock(
      rewardAddress,
      feeAddress,
      this.blockchain,
    );
    process.execArgv = R.reject(
      (item) => item.includes("debug"),
      process.execArgv,
    );
    ...
    const thread = spawn(function (input, done) {
      /*eslint-disable */
      require(input.__dirname + "/../util/consoleWrapper.js")(
        "mine-worker",
        input.logLevel,
      );
  ...
  static generateNextBlock(rewardAddress, feeAddress, blockchain) {
    ...
    const transactionsInBlocks = R.flatten(
      R.map(R.prop("transactions"), blocks),
    );
    const inputTransactionsInTransaction = R.compose(
      R.flatten,
      R.map(R.compose(R.prop("inputs"), R.prop("data"))),
    );
    const difficulty = blockchain.getDifficulty(index);
    ...
    R.forEach((transaction) => {
      ...
      let transactionInputFoundAnywhere = R.map((input) => {
        ...
        let wasItFoundInSelectedTransactions = R.not(
          R.isNil(
      ...
      if (R.all(R.equals(false), transactionInputFoundAnywhere)) {
        if (
          (transaction.type === "regular" ||
            transaction.type === "create_event_wallet" ||
    ...
    let transactions = R.defaultTo(
      [],
      R.take(Config.TRANSACTIONS_PER_BLOCK, selectedTransactions),
    );
    ...
    //console.info(`Block generated: ${JSON.stringify(b)}`);
    //
    return b;
  }
  ...
  static proveWorkFor(jsonBlock, difficulty) {
    ...
    //console.info(
    //  `proving block: ${JSON.stringify(block)} with difficulty: ${difficulty}`,
    //);
    ...
    do {
      ...
      //console.info(
      //  `\nnonce: ${block.nonce} \nHash: ${block.hash} \nblockDifficulty: ${blockDifficulty} vs difficulty: ${difficulty}`,
      //);
    } while (blockDifficulty >= difficulty);
    console.info(
      `Block found: time '${process.hrtime(start)[0]} sec' dif '${difficulty}' hash '${block.hash}' nonce '${block.nonce}'`,
    );
    return block;
  }
}
