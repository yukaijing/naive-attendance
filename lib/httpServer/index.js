...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        this.app.post('/operator/wallets', (req, res) => {
            ...
            if (!/^\d{8}[a-zA-Z]$/.test(studentID)) throw new HTTPError(400, 'studentID must be eight numbers followed by a character');

            studentID = studentID.slice(0, 8) + studentID.slice(8).toLowerCase();

            let existingWallet = operator.getWalletById(studentID);
            if (existingWallet != null) throw new HTTPError(409, `Wallet for student '${studentID}' already exists`);
            ...
            res.status(201).send({ walletId: walletId, address: newAddress });
        ...
        this.app.post('/operator/wallets/:walletId/transactions', (req, res) => {
            ...
            try {
                ...
                let newTransaction = operator.createTransaction(walletId, req.body.fromAddress, req.body.toAddress, req.body.amount, req.body['changeAddress'] || req.body.fromAddress, req.body.studentId, req.body.eventId);
    ...
    registerWallet(studentId, publicKey, privateKey) {
        // Create the transaction object
        const transaction = new Transaction('register', studentId, publicKey);
