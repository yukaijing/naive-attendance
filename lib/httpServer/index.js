...
class HttpServer {
  constructor(node, blockchain, operator, miner) {
    ...
    this.app.post("/blockchain/transactions", (req, res) => {
      ...
      if (transactionFound != null)
        throw new HTTPError(
          409,
          `Transaction '${requestTransaction.id}' already exists`,
        );
    ...
    this.app.post("/operator/wallets", async (req, res) => {
      ...
      if (eventID) {
        ...
        if (existingWallet != null) {
          res
            .status(409)
            .send({ error: `Wallet for event '${eventID}' already exists` });
          return;
        }
        ...
        try {
          ...
          let transactionResponse = await this.createEventWalletTransaction(
            eventID,
            publicKey,
          );
        ...
        if (!/^\d{8}[a-zA-Z]$/.test(studentID)) {
          res.status(400).send({
            error: "studentID must be eight numbers followed by a character",
          });
          return;
        }
        ...
        if (existingWallet != null) {
          res.status(409).send({
            error: `Wallet for student '${studentID}' already exists`,
          });
          return;
        }
        ...
        try {
          ...
          // Create a transaction for the new wallet
          let transactionResponse = await this.createWalletTransaction(
            walletId,
            publicKey,
            studentID,
            password,
    ...
    this.app.post("/operator/wallets/:walletId/transactions", (req, res) => {
      ...
      if (password == null)
        throw new HTTPError(401, "Wallet's password is missing.");
      ...
      try {
        ...
        if (
          ex instanceof ArgumentError ||
          ex instanceof TransactionAssertionError
        )
          throw new HTTPError(400, ex.message, walletId, ex);
        else throw ex;
      }
    ...
    this.app.post("/miner/mine", (req, res, next) => {
      ...
      const feeAddress = req.body["feeAddress"] || rewardAddress;
      miner
        .mine(rewardAddress, feeAddress)
        .then((newBlock) => {
          newBlock = Block.fromJson(newBlock);
          blockchain.addBlock(newBlock);
          axios.post(`http://localhost:3001/miner/mine`, {
        ...
        .catch((ex) => {
          if (
            ...
            ex.message.includes("Invalid index")
          )
            next(
              new HTTPError(
                409,
  ...
  listen(host, port) {
    return new Promise((resolve, reject) => {
      this.server = this.app.listen(port, host, (err) => {
        if (err) reject(err);
        console.info(
          `Listening http on port: ${this.server.address().port}, to access the API documentation go to http://${host}:${this.server.address().port}/api-docs/`,
        );
        resolve(this);
      });
    });
  ...
  async takeAttendanceTransaction(
    walletId,
    publicKey,
    studentId,
    eventId,
  ...
  async createEventWalletTransaction(eventID, publicKey) {
    return new Promise((resolve, reject) => {
      ...
      fetch(
        `http://localhost:3001/operator/wallets/${eventID}/transactions`,
        options,
      )
        .then((response) => response.json())
        .then((data) => resolve(data))
